...
1. Initialisation projet:
   - `ng new dev-portfolio --routing --style=scss`.
   - Configurer structure: `core/`, `shared/`, `features/sections`, `features/three-scene`. [itnext](https://itnext.io/choosing-a-highly-scalable-folder-structure-in-angular-d987de65ec7)
2. Routing:
   - Route racine `/` avec layout principal (navbar, footer, scroll container).
   - Sections comme composants dans une page unique (type single-page scrolling).
3. Design système:
   - Palette de couleurs (clair/sombre).
   - Typographie (1–2 fonts max).
   - Composants UI réutilisables dans `shared/components` (primary-button, section-title, tag, card). [itnext](https://itnext.io/choosing-a-highly-scalable-folder-structure-in-angular-d987de65ec7)
4. Content model:
   - Créer des interfaces TypeScript:
     - `Education`, `Experience`, `Project`, `Service`.
   - Créer des fichiers JSON/TS statiques pour les données (pour ne pas les hardcoder dans les templates).
